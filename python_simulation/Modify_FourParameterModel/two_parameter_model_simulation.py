...
def actualFigure(anchor_x, anchor_y, target_x, target_y, parm_array_x, parm_array_y,string):
    ...
    plt.xlim(-6, 6)
    plt.ylim(-6, 6)
    # plt.text(-5.8, 5.5, r'$\mu=0.5,\ \sigma=0$')  # 在x轴-4， y轴0.30开始加入说明
    # plt.text(-5.8, 5.5, r'$\sigma=0,\ \mu=2.0$')  # 在x轴-4， y轴0.30开始加入说明
    # plt.text(-3.7, 5.5, ",rmse="+RMSE)  # 在x轴-4， y轴0.
    global tim
    plt.text(-5.8, 5.5, u"陷入局部最优解次数="+str(round(cunt,2))+"，"+u"定位时间=15.03,"+u"rmse=" + str(RMSE))  # 在x轴-4， y轴0.30开始加入说明
...
for i in range (NumberOfTargetNode):
    ...
    for j in range(NumberOfAnchorNode):
        ...
        # d = addGaussNoise(d, 0, 2.0) #添加均值为0， 标准差为1的高斯噪声
        d2.append(d*d)
...
tim = round(tim,2)
RMSE = actualFigure(anchor_x, anchor_y, target_x, target_y, parm_array_x, parm_array_y, u"两参数模型群组规模20，迭代次数100")
# RMSE_barrier = actualFigure(anchor_x, anchor_y, target_x, target_y, parm_array_barier_x, parm_array_barier_y, u"7.5米异常误差存在时")
print "RMSE=", RMSE
