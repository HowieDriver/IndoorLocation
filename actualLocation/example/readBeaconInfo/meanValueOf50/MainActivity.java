...
public class MainActivity extends AppCompatActivity {
    ...
    Queue<Integer> queRssi = new LinkedList<Integer>();
    @Override
...
//                Log.w(TAG, "rssi:" + rssi);
                ...
                //((majorBytes[0]==0x13 || majorBytes[1]==0x09) && (minorBytes[0]==0x68 || minorBytes[1]==0x6f)) {
...
//                    Log.w(TAG, "txPowerBytes:" + txPowerString);
//                    Log.w(TAG, "rssi:" + rssi);
                    Log.w(TAG, "queRssi.size()=" + queRssi.size());
                    float sum = 0;
                    float avg = 0;
                    for(int q : queRssi){
                        sum += q;
                    }
                    avg = sum/queRssi.size();
                    avg = (int)(avg*100+0.5) / 100.0f;
                    if(queRssi.size()<50) {
                        beacon3.setText("小于50个数当前均值："+String.valueOf(avg));
                    }
                    else {
                        queRssi.poll();
                        beacon3.setText(String.valueOf(avg));
                        save(avg + ",");
                    }
                    Log.w(TAG, "当前均值=" + String.valueOf(avg));
