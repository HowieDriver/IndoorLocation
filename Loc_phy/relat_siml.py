...
anchor_x = [-5,  0, 5,   0, 0]
anchor_y = [  0, 5,  0, -5, 0]
target_x = np.random.uniform(-5, 5, size=(10))#随机生成50个数，范围在-5到5
target_y = np.random.uniform(-5, 5, size=(10))
...
for i in range (10):
    # # 绘制实际锚节点和单个目标节点位置
    # plt.scatter(anchor_x, anchor_y, color='green', marker='o')  # 离散点
    # plt.scatter(target_x[i], target_y[i], color='blue', marker='^')  # 离散点
    # plt.show()
    D2 = []
    for j in range(5):
    ...
    D2 = np.mat(P)
    E = np.mat(np.eye(6, 6, dtype=int))
    l = np.mat(np.ones((6, 1)))
    I = l * (l.T)
    H = E - (1.0/6.0) * I
    #print "H=", H
    B = -0.5 * H * D2 * H
